...
tran <- make_transition3(lakes_crop, res = c(0.001, 0.001))
...
pos2 <- interpolate_path(dtc, trans = tran$transition, start_time = "2018-08-01 00:00:00", int_time_stamp = (86400/3), out_class = NULL, lnl_thresh = 2, show_progress = FALSE)
...
out <- rbind(foo, bar)
out[record_type == "receiver", colr := "black"][record_type == "detection", colr := "red4"][record_type == "interpolated", colr := "red4"]
out[record_type == "receiver", size := 3.5][record_type == "detection", size := 3.5][record_type == "interpolated", size := 3.5]
...
animate(p, duration = 60, start_pause = 10, end_pause = 10, width = 800, height = 450, fps = 20)

# one minute animation
anim_save(filename = "images/animation.gif")
