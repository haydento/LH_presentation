...
fish_sum <- summarize_detections(det = dtc, location_col = "glatos_array2", receiver_locs = recs, summ_type = "location")
fish_sum <- fish_sum[, c("glatos_array2", "num_fish", "num_dets")]


coords <- unique(recs[, c("glatos_array2", "deploy_lat", "deploy_long")], by = c("glatos_array2", "deploy_lat", "deploy_long"))

coords <- coords[, .(deploy_long = mean(deploy_long), deploy_lat = mean(deploy_lat)), by = .(glatos_array2)]
...
geom_sf(data=fish_sum_sf, inherit.aes = FALSE, show.legend = FALSE, size = 10, shape = 21, color = "black", stroke = 2, fill = NA) +
  ...
  ggsave(plot = p, filename = file.path("images", "overall.png"), dpi = "screen", width = 16, height = 9)
